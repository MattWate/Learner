...
(function(){
  ...
  const loadAuthenticatedFlow=()=>{
    ...
    const s=document.createElement('script');s.src='/shared/onboarding.js';document.body.appendChild(s);
  };
  ...
  async function init(){
    ...
    if(session){
      ...
      if(params.get('mode')==='family'&&params.get('add')==='1'&&!slotState.canAdd){location.replace('/app.html');return;}
      loadAuthenticatedFlow();
    } else showPublicRoleChoice();
  }
