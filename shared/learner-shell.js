...
(function () {
    const NAV_ITEMS = [
        ...
        { group: 'PROGRESS', label: 'Activity History', icon: 'history', path: '/app.html#history', key: 'history' },
            ...
            <div class="lg-app">
                <aside class="lg-sidebar" aria-label="Main navigation">
                    ...
                    <div class="lg-learner-card">
                        ...
                        <a class="lg-profile-link" href="/app.html" title="Switch learner" aria-label="Switch learner"><i data-lucide="chevrons-up-down" width="17"></i></a>
                    </div>
                    <nav class="lg-nav">${navHtml(options.activeKey)}</nav>
                    <div class="lg-sidebar-bottom">
                        <div class="lg-plan-card"><strong>${escapeHtml(accountLabel(account))}</strong><span>Learner profile active</span></div>
                        <nav class="lg-nav">
                            <a class="lg-nav-link" href="${profileLink('/app.html#settings')}"><i data-lucide="settings" width="18"></i><span>Settings</span></a>
                            <button class="lg-nav-link" id="lg-sign-out" type="button" style="width:100%;border:0;background:transparent;text-align:left;cursor:pointer;"><i data-lucide="log-out" width="18"></i><span>Sign out</span></button>
                        </nav>
                    </div>
                </aside>
                <main class="lg-main">
                    <header class="lg-mobile-header">
                        <div class="lg-mobile-brand"><img src="/logo.svg" alt=""><div><strong>${escapeHtml(title)}</strong><span>${escapeHtml(profileName)}'s workspace</span></div></div>
                        <button class="lg-menu-button" type="button" data-lg-open-menu aria-label="Open navigation"><i data-lucide="menu"></i></button>
                    </header>
                    ${options.content}
                </main>
            </div>`;
